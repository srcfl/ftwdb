...
pub struct IntegrityReport {
    ...
    pub raw_points: u64,
    pub raw_commits: u64,
    /// Bytes recovery removed (writable open) or virtually removed (read-only
    /// open) from the raw log tail, with [`IntegrityReport::raw_recovered_tail`]
    /// naming why: a short tail is the expected power-loss signature, while a
    /// checksum mismatch indicates media corruption. See
    /// [`crate::RecoveredTail`].
